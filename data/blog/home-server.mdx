...
series: 'שרת ביתי'
seriesOrder: 5
seriesLabel: 'הגרסה הראשונה (2020)'
type: Blog
---
...
*[Seerr](https://docs.seerr.dev/)* (עד לא מזמן קראו לו Jellyseerr) מרכז את הכל למקום אחד: גם סרטים וגם סדרות, בממשק נוח אחד שנראה כמו נטפליקס. אני מחפש מה שבא לי, לוחץ "בקש לצפות", והבקשה עוברת מאחורי הקלעים ל*Sonarr* או ל*Radarr* שמתחילים בתהליך שתיארתי למעלה. בסוף התוכן פשוט מופיע ב*Jellyfin*.

יש גם אפליקציות לטלפון שמשלבות את *Jellyfin* ו*Seerr* לחוויה אחת, אבל האמת שאני נשארתי עם האפליקציה הרשמית של *Jellyfin*. הבעיה עם האפליקציות החדשות היא שבגלל הAI צצות יותר מדי כאלה, ואי אפשר לעקוב איזו מהן מתאימה לאיזו דרישה.
...
services:
  ...
  seerr:
    # media request and discovery manager for Jellyfin (successor to Jellyseerr)
    ...
    ports:
      - "5055:5055"
    volumes:
      # reuses the former jellyseerr config volume; Seerr auto-migrates on first start
...
volumes:
  ...
  radarr:
  bazarr:
  jellyseerr: # Seerr config (name kept to preserve existing data)
  cleanuparr:
```
