...
### עדכון (יוני 2026): הצינור באמת רץ

כשכתבתי את הפוסט הזה לפני שנה, סיימתי במשפט ש״אלו רק תוכניות ולא באמת סיימתי״. אז הנה עדכון: התקדמתי, וכל ה budget tracking pipeline כבר רץ אצלי בשרת מדי יום. שווה לי לתאר אותו מקצה לקצה, כי כל חוליה כבר עובדת:
...
- **הנתונים פתוחים לAI.** הMCP שלActual Budget חושף את התקציב לסוכן: כמה הוצאתי, באיזו קטגוריה, מתי, ומה עוד פתוח החודש.
- **סוכן בn8n שמדבר איתי.** הAI יושב בn8n, ואני מדבר איתו בטלגרם בעברית רגילה. ״כמה הוצאתי על אוכל החודש?״, ״תראה לי תנועות חריגות מהשבוע״, ״תסכם לי את החודש שעבר מול הקודם״. הוא שולף, מחשב, ועונה.

מה השתנה מאז? בעיקר הדרך שבה אני בונה ומתחזק את הצינור הזה. במקום לכתוב כל workflow ביד, אני נותן לסוכן קוד לבנות אותם בעצמו בn8n דרך MCP, וכל ריצה נשמרת בתור execution שאפשר לתעד ולתקן כשמשהו נשבר. הרחבתי על כל הגישה הזאת [בפוסט נפרד על n8n וMCP](/blog/ai-deterministic-n8n), והיא בדיוק מה שהפך את ה״תוכניות״ למשהו שבאמת רץ.

ומה אפשר לעשות עם זה היום? בקצרה, להפסיק לפתוח אקסלים. כל מה שפעם היה ״שב בסוף החודש ועבור על הכל ידנית״ הפך לשאלה אחת בטלגרם. וכי הכל פתוח אצלי בשרת, אני יכול להמשיך לחבר עוד קוביות: התראה כשחורגים מתקציב בקטגוריה, סיכום שבועי שמגיע לבד, או חיבור לעוד מקורות מידע. תכל'ס, מפה זה רק שאלה של דמיון (וכמה זמן יש לי).

הנה הסוכן עצמו, בתור workflow אמיתי שאתם מוזמנים לפתוח ולחקור:
