...
ragQuestions:
  ...
  - 'איך מקבלים התראה כשמשהו משתנה ב-API חיצוני?'
  - 'אפשר להשתמש ב-Postman כדי להריץ בדיקות API מתוזמנות?'
  - 'איך שומרים state בין הרצות של אוטומציה ללא קוד?'
  - 'איך עושים polling ל-API ושולחים התראה כשיש שינוי?'
type: Blog
---
