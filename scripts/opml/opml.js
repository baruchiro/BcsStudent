...
const ignoreCategories = ['אחרים', 'מדע', 'StackOverflow', 'מיקרוסופט', 'English']

const postPath = path.join(__dirname, '..', '..', 'content', 'posts', 'rss-blogs', 'rss-blogs.md')
...
const getTitle = async (url) => axios.get(url).then(({ data }) => {
...
const createFeed = (name, url, title, feed) => {
    ...
    const formattedName = `${name}`.startsWith('<div') ? name.match(/>(.*)</)[1] : name
    ...
    .then((opmlObj) => {
        ...
        return opmlObj.opml.body[0].outline
            ...
            .reduce(async (post, category) => {
                ...
                content += await Promise.all(category.feeds.map(async (feed) => {
                    const title = await getTitle(feed.url).catch(({ status, statusText, code }) => {
                        console.warn(`Error with ${feed.name} ${feed.url}`, { status, statusText, code })
                        return ''
                    })
